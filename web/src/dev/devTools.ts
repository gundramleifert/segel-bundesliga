 ...
 * `VITE_DEV_TOOLS=true`, which `render.yaml` sets for the public **test** instance
 * (`sbl-web`), whose backend runs with `SBL_DEV_LOGIN=true` anyway. It stays false in any
 * build that does not ask for it, which is what keeps a real deployment free of a panel
 * that signs anyone in as an administrator.
 *
