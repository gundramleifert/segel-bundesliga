      ...
      {anchor &&
        createPortal(
          <div
            ...
            className="pointer-events-none fixed z-[60] -translate-x-1/2 -translate-y-full rounded-lg bg-slate-900/95 px-2.5 py-1.5 text-xs font-medium text-white shadow-lg"
          >
            {text}
            {/* The little point. `border` rather than a rotated square: a rotated element
                inside a translated one picks up the parent's transform origin and drifts. */}
            <span
              aria-hidden
              className="absolute left-1/2 top-full -ml-1 border-4 border-transparent border-t-slate-900/95"
            />
          </div>,
          document.body,
        )}
