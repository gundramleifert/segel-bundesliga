...
class TestOncePerCompetition:
    ...
    async def test_endpoint_rejects_foreign_team(self, client, caplog):
        ...
        from tests.stories.test_aufstellung import als_vereinsleitung, erste_mannschaft, spieltag_id

        team_id, club_id, squad = await erste_mannschaft()
        headers = await als_vereinsleitung(client, caplog, "mv1@example.com", club_id)
