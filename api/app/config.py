...
# the mechanism, not a convention we invented. It's used only for the fields that are
# genuine credentials (jwt_secret, smtp_password); everything else stays a plain,
# non-sensitive env var. An explicit env var still wins if both are set, and the fields
# fall back to their defaults if the file is absent (e.g. local development, where the
# directory itself usually doesn't exist at all). Guarded on existence purely to skip
# pydantic-settings' otherwise-harmless "directory does not exist" warning locally.
